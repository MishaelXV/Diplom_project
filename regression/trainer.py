...
def optimize_params(Pe, A, sigma, N, boundary_dict):
    def objective(trial):
        window_size = trial.suggest_int("window_size", 3, 21)
        min_slope = trial.suggest_float("min_slope", 0.01, 0.5)
...
def collect_training_data():
    ...
    param_grid = list(itertools.product(
        np.linspace(500, 5000, 5),  # Pe[0]
        np.linspace(1, 10, 5),      # A
        np.linspace(0.0001, 0.01, 10),     # sigma
        [50, 100, 200],            # N
    ))
